...
def get_id_list_p_adic(args, label_path, a=0.6, b=0.8):
    '''
    ...
    if args.cross_val == 'fixed' or args.cross_val == 'mccv':  # fixed split or Monte Carlo cross-validation
        ...
        fold_size_mci = len(mci_list) / 5
        fold_size_de = len(de_list) / 5
        fold_size_sc = len(sc_list) / 5
        seed = (args.seed-41) % 5
        ...
        test_ids = (hc_list[int(seed * fold_size_hc):int((seed + 1) * fold_size_hc)] +
                    ...
                    mci_list[int(seed * fold_size_mci):int((seed + 1) * fold_size_mci)] +
                    de_list[int(seed * fold_size_de):int((seed + 1) * fold_size_de)] +
                    sc_list[int(seed * fold_size_sc):int((seed + 1) * fold_size_sc)])
        ...
        if seed == 4:  # take the first fold as validation set when seed is 4
            val_ids = ((hc_list[:int(fold_size_hc)] + ad_list[:int(fold_size_ad)]) +
                       mci_list[:int(fold_size_mci)]) + de_list[:int(fold_size_de)] + sc_list[:int(fold_size_sc)]
        else:
            val_ids = (hc_list[int((seed + 1) * fold_size_hc):int((seed + 2) * fold_size_hc)] +
                       ad_list[int((seed + 1) * fold_size_ad):int((seed + 2) * fold_size_ad)] +
                       mci_list[int((seed + 1) * fold_size_mci):int((seed + 2) * fold_size_mci)] +
                       de_list[int((seed + 1) * fold_size_de):int((seed + 2) * fold_size_de)] +
                       sc_list[int((seed + 1) * fold_size_sc):int((seed + 2) * fold_size_sc)])
...
class PADICLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_p_adic(args, self.label_path, a, b)
        ...
        if args.classify_choice == 'ad_vs_hc':  # 1 vs 0
            ...
            print('Delete MCI, Dep, and Schiz subjects in train ids list for AD vs HC classification')
            mask = (self.y[:, 0] < 2)
            self.X = self.X[mask]
            self.y = self.y[mask]
        elif args.classify_choice == 'ad_vs_nonad':
            # all other diseases are also 0, change labels to 0
            print('Change MCI, Dep, and Schiz subjects from 2 to 0 in train ids list for AD vs non-AD classification')
            self.y[:, 0] = np.where(self.y[:, 0] > 1, 0, self.y[:, 0])
        elif args.classify_choice == 'hc_vs_abnormal':
            # change all other diseases to 1
            print('Change MCI, Dep, and Schiz subjects from 2 to 1 in train ids list for HC vs abnormal classification')
