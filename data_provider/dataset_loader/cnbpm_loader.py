...
def get_id_list_cnbpm(args, label_path, a=0.6, b=0.8):
    ...
    if args.cross_val == 'fixed' or args.cross_val == 'mccv':  # fixed split or Monte Carlo cross-validation
        ...
        if seed == 4:  # take the first fold as validation set when seed is 4
            val_ids = (hc_list[:int(fold_size_hc)] + ad_list[:int(fold_size_ad)]) + mci_list[:int(fold_size_mci)]
        else:
...
class CNBPMLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        if args.classify_choice == 'ad_vs_hc':  # 1 vs 0
            # delete MCI
            print('Delete MCI subjects in train ids list for AD vs HC classification')
            mask = (self.y[:, 0] < 2)
            self.X = self.X[mask]
            self.y = self.y[mask]
        elif args.classify_choice == 'ad_vs_nonad':
            # all other diseases are also 0, change MCI label from 2 ot 0
            print('Change MCI label from 2 to 0 in train ids list for AD vs non-AD classification')
            self.y[:, 0] = np.where(self.y[:, 0] > 1, 0, self.y[:, 0])
        elif args.classify_choice == 'hc_vs_abnormal':
            # change all other diseases to 1
            print('Change MCI label from 2 to 1 in train ids list for HC vs abnormal classification')
