...
def get_id_list_caueeg(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for CAUEEG data
...
class CAUEEGLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        if args.classify_choice == 'ad_vs_hc':  # 1 vs 0
            ...
            print('Change MCI and other subjects from 2 to 0 in train ids list for dementia vs non-dementia classification')
            self.y[:, 0] = np.where(self.y[:, 0] > 1, 0, self.y[:, 0])
        elif args.classify_choice == 'hc_vs_abnormal':
            # change all other diseases to 1
            print('Change MCI and other subjects from 2 to 1 in train ids list for HC vs abnormal classification')
